...
* Sign-in failures — Antigravity eligibility text and a Codex device-code screen classify as account confirmation, and the dashboard says to open the CLI, complete the browser sign-in, and select Retry. The browser link stays off the banner and out of the log.
