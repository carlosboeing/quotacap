...
- `raw` not stored. The `raw` column is not present in the schema. Older databases are migrated by rebuilding the table in `src/store/db.ts:37-48`. `mapRow` deletes `raw` (`src/store/quotas.ts:18`). `getAllLatest` and `GET /api/quotas` and MCP `get_quotas` never return it. This is covered by `tests/http/api.test.ts` and `tests/store/db.test.ts`. Provider display fields (`displayName`, `vendor`, `harness`, `description`) are recomputed per request from a static registry: never persisted, never a lookup key, and carried as additive output-only fields on `/api/state` and MCP `get_quotas`. `ParsedQuota.raw` is an in-memory debug slice that is never written to disk — PTY adapters cap at 4096 chars, `claude` keeps the full result text.
