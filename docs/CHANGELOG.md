...
- Store: drop `raw` column and migrate old databases by rebuilding the table (`src/store/db.ts`), `mapRow` strips `raw` so `GET /api/quotas` and MCP never return it, add `credits_usd` and `resets_at_estimated`, `0700`/`0600` on `~/.quotacap` and `quotacap.db` (`fix/drop-raw` #11).
