...
Do not open a public issue for a vulnerability. Report privately through GitHub's private
vulnerability reporting (`Security → Advisories → New draft advisory`). If you
cannot use private reporting, contact [@carlosboeing](https://github.com/carlosboeing)
via GitHub. Include `QuotaCap security` in the subject, or open a draft advisory at
https://github.com/carlosboeing/quotacap/security/advisories/new.
...
- `~/.local/share/opencode/auth.json` — **only when you explicitly enable OpenCode Go** (`quotacap providers enable opencode-go`). Read in-memory, read-only, once per poll, entry-scoped to `opencode-go` (fallback `opencode`). Never written, copied, logged, or stored. Consent is recorded in config (`opencodeGoConsentAt`) and revoked by disabling. This is the single deliberate exception to credential-free polling, asserted read-only by `tests/adapters/credential-free.test.ts`.
- No network beyond the local loopback handler — except the `claude`/`agy`/`codex`/`kimi`/`grok` binaries themselves contacting their own vendors when QuotaCap spawns them. When OpenCode Go is enabled, QuotaCap itself also calls `https://opencode.ai/zen/go/v1/usage` with that provider's auth key — the only outbound request QuotaCap makes directly.

## What QuotaCap does not touch

- No tokens owned. It never reads `~/.codex/auth.json` (`CODEX_HOME`), `~/.kimi-code/credentials/kimi-code.json` or `~/.kimi/credentials/kimi-code.json`, `~/.grok/auth.json` (`GROK_HOME`), or `~/.gemini/oauth_creds.json`. It never uses `refresh_token` or `grant_type=refresh_token`. It has no hardcoded client ids. Those OAuth paths were removed in #14. This is asserted by `tests/adapters/credential-free.test.ts`. The single scoped exception is the opt-in OpenCode Go read described above; the static gate allows the `auth.json` literal in exactly two pinned files — `src/adapters/opencode-go.ts` (the adapter that reads it) and the generated dashboard bundle `src/webAssets.ts` (which embeds the consent copy naming it) — and the runtime spies assert the read is read-only and entry-scoped.
